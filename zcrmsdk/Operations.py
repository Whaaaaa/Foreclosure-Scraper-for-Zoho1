...
class ZCRMRecord(object):
    ...
    def convert(self, potential_record=None, assign_to_user=None):
        if self.entity_id is None:
            try:
                ...
                from .Utility import CommonUtil
            CommonUtil.raise_exception('Convert_Record', "id should be set for the record", 'ID DOES NOT PROVIDED',
                                       "ID")
        try:
            from .Handler import EntityAPIHandler
        except ImportError:
    ...
    def upload_link_as_attachment(self, link_url):
        if link_url is None:
            try:
                ...
                from .Utility import CommonUtil
            CommonUtil.raise_exception('Upload_Link_As_Attachment', "link URL must be given", 'URL_LINK NOT PROVIDED',
                                       "URL_LINK")
    ...
    def add_relation(self, junction_record):
        if junction_record is None:
            try:
                ...
                from .Utility import CommonUtil
            CommonUtil.raise_exception('add_relation', "junction_record must be given", 'JUNCTION RECORD NOT PROVIDED',
                                       "JUNCTION RECORD")
    ...
    def remove_relation(self, junction_record):
        if junction_record is None:
            try:
                ...
                from .Utility import CommonUtil
            CommonUtil.raise_exception('remove_relation', "junction_record must be given",
    ...
    def add_note(self, note_ins):
        if note_ins is None:
            try:
                ...
                from .Utility import CommonUtil
            CommonUtil.raise_exception('add_note', "note instance must be given", 'NOTE INSTANCE NOT PROVIDED',
                                       "NOTE INSTANCE")
    ...
    def update_note(self, note_ins):
        if note_ins is None or note_ins.id is None:
            try:
                ...
                from .Utility import CommonUtil
            CommonUtil.raise_exception('update_note', "note instance and note id must be given",
    ...
    def delete_note(self, note_ins):
        if note_ins is None or note_ins.id is None:
            try:
                ...
                from .Utility import CommonUtil
            CommonUtil.raise_exception('delete_note', "note instance and note id must be given",
                                       'NOTE INSTANCE OR NOTE ID NOT PROVIDED', "NOTE INSTANCE OR ID")
    ...
    def get_relatedlist_records(self, relatedlist_api_name, sort_by=None, sort_order=None, page=1, per_page=20):
        return ZCRMModuleRelation.get_instance(self, relatedlist_api_name).get_records(sort_by, sort_order, page,
                                                                                       per_page)
...
class ZCRMUser(object):
    ...
    defaultKeys = ["Currency", "Modified_Time", "created_time", "territories", "reporting_to", "Isonline", "created_by",
                   "Modified_By", "country", "id", "name", "role", "customize_info", "city", "signature", "name_format",
                   "language", "locale", "personal_account", "default_tab_group", "alias", "street", "theme", "state",
                   "country_locale", "fax", "first_name", "email", "zip", "decimal_separator", "website", "time_format",
                   "profile", "mobile", "last_name", "time_zone", "zuid", "confirm", "full_name", "phone", "dob",
                   "date_format", "status"]
...
class ZCRMModuleRelation(object):
    ...
    def get_records(self, sort_by_field=None, sort_order=None, page=1, per_page=20):
        try:
            ...
            from .Handler import RelatedListAPIHandler
        return RelatedListAPIHandler.get_instance(self.parent_record, self).get_records(sort_by_field, sort_order, page,
                                                                                        per_page)
    ...
    def get_notes(self, sort_by, sort_order, page, per_page):
        try:
            ...
            from .Handler import RelatedListAPIHandler
        return RelatedListAPIHandler.get_instance(self.parent_record, self).get_notes(sort_by, sort_order, page,
                                                                                      per_page)
